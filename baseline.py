...
def list_to_vector_array(file_list,
    ...
    # 02 Initialize dataset as empty array in case file_list is empty
    dataset = numpy.zeros((0, dims), float)

    # 03 Check if file_list is empty
    if len(file_list) == 0:
        print("Warning: file_list is empty - returning empty dataset")
        return dataset

    # 04 loop of file_to_vectorarray
...
if __name__ == "__main__":
    ...
    # dirs = sorted(glob.glob(os.path.abspath("{base}/min6dB/-6_dB_fan/fan/*".format(base=param["base_directory"]))))
    ...
    for dir_idx, target_dir in enumerate(dirs):
        ...
        # For Keras 3.x - handle both file extensions
        model_weights_file = model_file
        if model_file.endswith('.hdf5'):
            model_weights_file = model_file.replace('.hdf5', '.weights.h5')
        ...
        if os.path.exists(model_weights_file):
            ...
            # For Keras 3.x - use the correct file extension
            if model_file.endswith('.hdf5'):
                model_file = model_file.replace('.hdf5', '.weights.h5')
            model.save_weights(model_file)
